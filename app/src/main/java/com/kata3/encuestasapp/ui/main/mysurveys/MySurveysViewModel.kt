...
class MySurveysViewModel(private val surveyRepository: SurveyRepository) : ViewModel() {
    ...
    fun fetchSurveys(token: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val surveys = surveyRepository.getMySurveys(token)
                if (surveys != null) {
                    _surveyList.postValue(surveys)
                    _errorMessage.postValue(null)
                } else {
                    _errorMessage.postValue("Failed to load surveys")
                }
            } catch (e: Exception) {
